...
import {networkRequestAllowed} from './network-policy.mjs';
export function simulateFetch(permissions,fixtures,request) {
  ...
  if(!networkRequestAllowed(permissions.network??[],request))return failure('capability_denied','Network permission is not declared or the request route is not allowed.');
